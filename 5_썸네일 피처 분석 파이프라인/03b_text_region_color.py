...
EasyOCR 텍스트 ROI와 배경 ROI에서 색상 피처를 추출합니다.

의존성: easyocr, opencv-python-headless (02_text_ratio.py와 동일)

출력 컬럼:
    text_color_entropy       - 텍스트 ROI hue Shannon entropy
    text_color_saturation    - 텍스트 ROI 평균 채도 (0~255)
    text_color_brightness    - 텍스트 ROI 평균 밝기 (0~255)
    text_color_hue_std       - 텍스트 ROI hue 순환 표준편차
    bg_color_entropy         - 비텍스트 ROI hue Shannon entropy
    text_bg_entropy_diff     - text_color_entropy - bg_color_entropy
    text_bg_saturation_diff  - text_color_saturation - bg_color_saturation
"""
...
CHECKPOINT = Path(__file__).parent / "checkpoints/03b_text_region_color.csv"
COLS = [
    "text_color_entropy",
...
def hue_entropy(hue_cv: np.ndarray, sat: np.ndarray, region_mask: np.ndarray,
                min_colored: int = 10) -> float:
...
def save_checkpoint_dict(cached: dict):
...
def main(args):
    ...
    if len(remaining) > 0:
        ...
        try:
            for i, (_, row) in enumerate(tqdm(remaining.iterrows(), total=len(remaining), desc="텍스트 영역 색상")):
                ...
                cached[row["video_id"]] = worker.extract(str(img_path))
                if (i + 1) % 50 == 0:
                    save_checkpoint_dict(cached)
        finally:
            worker.close()
        save_checkpoint_dict(cached)
    ...
    output_path = args.output or args.input.replace(".csv", "_text_region_color.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="텍스트/배경 ROI 색상 피처 추출")
