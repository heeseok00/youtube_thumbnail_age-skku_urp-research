"""
compare_age_groups.py
추출된 피처를 ~34세 이하 vs 65세+ 그룹 간 비교 분석 및 시각화

사용법:
    python analysis/compare_age_groups.py --input <피처가_추가된_csv>
    python analysis/compare_age_groups.py  # 기본값으로 실행
"""
...
def assign_age_group(df: pd.DataFrame) -> pd.DataFrame:
    """각 영상을 ~34세 / 65세+ 그룹으로 분류 (dominant 단일 버킷 기준)."""
    df = df.copy()
    ...
    df["age_group"] = df.apply(group, axis=1)
    return df


def compare_continuous(df: pd.DataFrame, feat: str, label: str, ax):
    """연속형 피처: Box plot + Mann-Whitney U test."""
    g0 = df[df["age_group"] == "~34세"][feat].dropna()
    g1 = df[df["age_group"] == "65세+"][feat].dropna()
...
def compare_categorical(df: pd.DataFrame, feat: str, label: str, ax):
    """범주형 피처: Stacked bar + Chi-square test."""
    sub = df[df["age_group"].isin(["~34세", "65세+"])][["age_group", feat]].dropna()
    if len(sub) < 10:
        ax.set_title(f"{label}\n(데이터 부족)")
        return None
...
def main(args):
    ...
    df = df_all[df_all["subcategory"] == "시사/뉴스/사건"].copy().reset_index(drop=True)
    print(f"\n[대상] 시사/뉴스/사건: {len(df):,}개 (전체 {len(df_all):,}개 중)")
    ...
    print(f"  제외(35~64세 dominant): {df['age_group'].isna().sum():,}개\n")

    sub = df[df["age_group"].isin(["~34세", "65세+"])]
    ...
    if avail_cont:
        ...
        if len(avail_cont) == 1:
            axes = [axes]
        fig.suptitle("연속형 피처 비교: ~34세 vs 65세+", fontsize=12, y=1.01)
        cont_results = []
        for ax, (feat, label) in zip(axes, avail_cont):
            result = compare_continuous(sub, feat, label, ax)
            if result:
    ...
    if avail_cat:
        ...
        if len(avail_cat) == 1:
            axes = [axes]
        fig.suptitle("범주형 피처 비교: ~34세 vs 65세+", fontsize=12, y=1.01)
        cat_results = []
        for ax, (feat, label) in zip(axes, avail_cat):
            result = compare_categorical(sub, feat, label, ax)
            if result:
    ...
    if avail_text and "text_ratio" in df.columns:
        ...
        if len(text_sub) >= 20:
            ...
            fig.suptitle("텍스트 ROI 색상 (text_ratio>0.05): ~34세 vs 65세+", fontsize=12, y=1.01)
            text_results = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="연령대별 썸네일 피처 비교 분석")
    parser.add_argument("--input", default=str(
        Path(__file__).parent.parent.parent / "Data/SOCIETY/SOCIETY_new_category_v3_add_thumnail_title_features_merged.csv"
    ))
