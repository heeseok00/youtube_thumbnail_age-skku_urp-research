...
Later visual measures still use ordinary cues such as people, text, and color. Grad-CAM was not used to invent a new feature set. It was used to decide which regions of the thumbnail those measures should focus on. We first trained a thumbnail-only classifier to distinguish content associated with people aged 34 and under (~34) from content associated with people aged 65 and over (65+), and used Grad-CAM (Selvaraju et al., 2017) to see where the model treated evidence as concentrated. If activation lies on overlay text, later analyses emphasize text area and text–background contrast. If it lies on people, they emphasize person area, count, expression, and head direction. If it spreads across the scene, they turn to captions of people, actions, and setting. Grad-CAM is an exploratory step. It guides where to measure; it is not treated as a conclusion about age-related style.
