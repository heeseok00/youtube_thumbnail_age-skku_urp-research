...
Test-set accuracy was .784 overall, .822 for EDU, .721 for HEALTH, .867 for LIFESTYLE, and .749 for SOCIETY. Recall was higher for 18–34 (.842) than for 65+ (.725), so more 65+ images were labeled 18–34 (257) than the reverse (150). Class-wise scores are in Table B1.

Table B1. Test-set prediction of the age-associated label.
...
Table B2 reports where predicted-class heatmap energy fell on the same top-50 correct images. In 65+ cases, text and person together held 56.1% of heatmap energy (text 31.0%, person 25.1%). Text occupied 33.9% of the frame, so it is a large part of the layout rather than a small hot spot (concentration = 0.86). Person occupied 20.9% of the frame but 31.4% of pixels with CAM ≥ 0.5 (concentration = 1.42), so discriminative information was more densely packed in that smaller region. In 18–34 cases, 72.4% of heatmap energy and 74.5% of high-activation pixels fell on the background.

Table B2. Grad-CAM energy by exclusive ROI (top-50 correct; predicted-class heatmap). Concentration is energy share divided by area. Overlapping pixels are assigned to text, then person, then background.
...
Table B3. Overlapping ROI area and area-normalized deletion/insertion (top-50 correct, n = 50 per group). Baseline probability was .999 for 65+ and 1.000 for 18–34.
...
Figure B2. Grad-CAM for correctly classified 65+ thumbnails (three per category; rows EDU, HEALTH, LIFESTYLE, SOCIETY).

[Figure B3 here]

Figure B3. Grad-CAM for correctly classified 18–34 thumbnails (three per category; rows EDU, HEALTH, LIFESTYLE, SOCIETY).
